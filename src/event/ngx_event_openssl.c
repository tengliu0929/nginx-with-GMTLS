...
ngx_ssl_create(ngx_ssl_t *ssl, ngx_uint_t protocols, void *data)
{
	...
	SSL_CTX_clear_options(ssl->ctx,SSL_OP_NO_NTLS);
    if(!(protocols & NGX_SSL_GMSSL)) {
		SSL_CTX_set_options(ssl->ctx, SSL_OP_NO_NTLS);
	}
#endif
...
ngx_int_t
ngx_gmssl_create(ngx_ssl_t *ssl, ngx_uint_t protocols, void *data)
{


	ssl->ctx = SSL_CTX_new(NTLS_method()); //gmssl
	SSL_CTX_enable_ntls(ssl->ctx);
	//SSL_CTX_enable_sm_tls13_strict(ssl->ctx);
	...
	SSL_CTX_set_min_proto_version(ssl->ctx,0);
    SSL_CTX_set_max_proto_version(ssl->ctx,NTLS_VERSION);
#endif
...
// gmssl
ngx_int_t
ngx_gmssl_sign_certificates(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_array_t *certs,
    ngx_array_t *keys, ngx_array_t *passwords)
{
    ngx_str_t   *cert, *key;
    ngx_uint_t   i;
    ...
    for (i = 0; i < certs->nelts; i++) {
        ...
        if (ngx_gmssl_sign_certificate(cf, ssl, &cert[i], &key[i], passwords)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
...
ngx_int_t
ngx_gmssl_sign_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *cert,
    ngx_str_t *key, ngx_array_t *passwords)
{
    char            *err;
    X509            *x509;
...
ngx_int_t
ngx_gmssl_enc_certificates(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_array_t *certs,
    ngx_array_t *keys, ngx_array_t *passwords)
{
    ngx_str_t   *cert, *key;
    ngx_uint_t   i;
    ...
    for (i = 0; i < certs->nelts; i++) {
        ...
        if (ngx_gmssl_enc_certificate(cf, ssl, &cert[i], &key[i], passwords)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
...
ngx_int_t
ngx_gmssl_enc_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *cert,
    ngx_str_t *key, ngx_array_t *passwords)
{
    char            *err;
    X509            *x509;
...
ngx_int_t
ngx_gmssl_sign_connection_certificate(ngx_connection_t *c, ngx_pool_t *pool,
    ngx_str_t *cert, ngx_str_t *key, ngx_array_t *passwords)
{
    char            *err;
    X509            *x509;
...
ngx_int_t
ngx_gmssl_enc_connection_certificate(ngx_connection_t *c, ngx_pool_t *pool,
    ngx_str_t *cert, ngx_str_t *key, ngx_array_t *passwords)
{
    char            *err;
    X509            *x509;
    ...
    ngx_uint_t prefer_server_ciphers)
{
    ...
    //ngx_ssl_error(NGX_LOG_EMERG,ssl->log,0,"SSL_CTX list %s\n",(char *)ciphers->data);
...
ngx_ssl_handshake(ngx_connection_t *c)
{
    ...
    // ssl version test
	{
	  int max_ver = 0, min_ver = 0;
     // int i = 0;
	  //STACK_OF(SSL_CIPHER) *ciphers = SSL_get_ciphers(c->ssl->connection);
	  ssl_get_min_max_version(c->ssl->connection,&min_ver,&max_ver,NULL);
	  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "The tls max_ver is %d, min_ver is %d", max_ver, min_ver);
      /*
	  for( i = 0; i < sk_SSL_CIPHER_num(ciphers); i++ )
	  	{
	  	  const SSL_CIPHER *cipher = sk_SSL_CIPHER_value(ciphers,i);
		  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0,"The tls c->min_tls %d, c->min_tls %d",cipher->min_tls,cipher->max_tls);
	  	}
	  */
    }
