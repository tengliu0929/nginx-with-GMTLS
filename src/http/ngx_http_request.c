...
ngx_http_ssl_certificate(ngx_ssl_conn_t *ssl_conn, void *arg)
{
    ...
    if(!sscf->gmssl_enable) {
	    ...
	    for (i = 0; i < nelts; i++) {
         ...
         //sign
         nelts = sscf->gmssl_sign_certificate_values->nelts;
		 certs = sscf->gmssl_sign_certificate_values->elts;
		 keys = sscf->gmssl_sign_key_values->elts;
		 ...
		 for( i = 0; i < nelts; i++ ) {
		   ...
		   ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, 
		   				  "gmssl cert: \"%s\"",cert.data);
          if(ngx_http_complex_value(r,&keys[i],&key) != NGX_OK ) {
           goto failed;
		  }
		 ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0,
									"gmssl key: \"%s\"", key.data);
		 if( ngx_gmssl_sign_connection_certificate(c, r->pool, &cert, &key,
		 									sscf->passwords) 
		 		!= NGX_OK)
		 	{
		 	  goto failed;
         ...
         //enc
         nelts = sscf->gmssl_enc_certificate_values->nelts;
		 certs = sscf->gmssl_enc_certificate_values->elts;
		 keys =  sscf->gmssl_enc_key_values->elts;
		 ...
		 for( i = 0; i < nelts; i++ ) {
		   ...
		   ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0,
									"gmssl key: \"%s\"", key.data);
           if( ngx_gmssl_enc_connection_certificate(c, r->pool, &cert, &key,
		 									sscf->passwords) 
		 		!= NGX_OK)
		 	{
		 	  goto failed;
