...
static ngx_command_t  ngx_http_proxy_commands[] = {
     ...
     //gmssl
	{
	  ...
	  ngx_conf_set_flag_slot,
	  NGX_HTTP_LOC_CONF_OFFSET,
	  offsetof(ngx_http_proxy_loc_conf_t, proxy_gmssl_enable),
	  NULL},
    { ngx_string("proxy_gmssl_sign_certificate"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_proxy_loc_conf_t, upstream.proxy_gmssl_sign_certificate),
      NULL },
    { ngx_string("proxy_gmssl_sign_key"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_proxy_loc_conf_t, upstream.proxy_gmssl_sign_key),
      NULL },
    { ngx_string("proxy_gmssl_enc_certificate"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_proxy_loc_conf_t, upstream.proxy_gmssl_enc_certificate),
      NULL },
    { ngx_string("proxy_gmssl_enc_key"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_proxy_loc_conf_t, upstream.proxy_gmssl_enc_key),
      NULL },
...
ngx_http_proxy_create_loc_conf(ngx_conf_t *cf)
{
	...
	conf->upstream.proxy_gmssl_sign_certificate = NGX_CONF_UNSET_PTR;
	conf->upstream.proxy_gmssl_sign_key = NGX_CONF_UNSET_PTR;
	conf->upstream.proxy_gmssl_enc_certificate = NGX_CONF_UNSET_PTR;
	conf->upstream.proxy_gmssl_enc_key = NGX_CONF_UNSET_PTR;
#endif
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
	...
	ngx_conf_merge_ptr_value(conf->upstream.proxy_gmssl_sign_certificate, prev->upstream.proxy_gmssl_sign_certificate, NULL);
	ngx_conf_merge_ptr_value(conf->upstream.proxy_gmssl_sign_key, prev->upstream.proxy_gmssl_enc_key, NULL);
	ngx_conf_merge_ptr_value(conf->upstream.proxy_gmssl_enc_certificate, prev->upstream.proxy_gmssl_enc_certificate, NULL);
	ngx_conf_merge_ptr_value(conf->upstream.proxy_gmssl_enc_key, prev->upstream.proxy_gmssl_enc_key, NULL);
...
ngx_http_proxy_set_ssl(ngx_conf_t *cf, ngx_http_proxy_loc_conf_t *plcf)
{
	...
	if(!plcf->proxy_gmssl_enable) {
       ...
       if(ngx_gmssl_create(plcf->upstream.ssl, plcf->ssl_protocols, NULL) 
	   		!= NGX_OK )
       	{
       	  return NGX_ERROR;
       	}
    ...
    if(!plcf->proxy_gmssl_enable) {
	        ...
	        && plcf->upstream.ssl_certificate->value.len)
	    {
	            ...
	            || plcf->upstream.ssl_certificate_key->lengths)
	        {
     	    ...
     	    if (plcf->upstream.proxy_gmssl_sign_certificate
	        && plcf->upstream.proxy_gmssl_sign_certificate->value.len)
	    {

		   if (plcf->upstream.proxy_gmssl_sign_key == NULL) {
	            ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
	                          "no \"proxy_ssl_certificate_key\" is defined "
	                          "for certificate \"%V\"",
	                          &plcf->upstream.proxy_gmssl_sign_certificate->value);
	            return NGX_ERROR;
	        }

	        if (plcf->upstream.proxy_gmssl_sign_certificate->lengths
	            || plcf->upstream.proxy_gmssl_sign_key->lengths)
	        {
	            plcf->upstream.ssl_passwords =
	                  ngx_ssl_preserve_passwords(cf, plcf->upstream.ssl_passwords);
	            if (plcf->upstream.ssl_passwords == NULL) {
	            ...
	            if (ngx_gmssl_sign_certificate(cf, plcf->upstream.ssl,
	                                    &plcf->upstream.proxy_gmssl_sign_certificate->value,
	                                    &plcf->upstream.proxy_gmssl_sign_key->value,
	                                    plcf->upstream.ssl_passwords)
	                != NGX_OK)
	            {
	                return NGX_ERROR;
	    ...
	    if (plcf->upstream.proxy_gmssl_enc_certificate
	        && plcf->upstream.proxy_gmssl_enc_certificate->value.len)
	    {
	        if (plcf->upstream.proxy_gmssl_enc_key == NULL) {
	            ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
	                          "no \"proxy_ssl_certificate_key\" is defined "
	                          "for certificate \"%V\"",
	                          &plcf->upstream.proxy_gmssl_enc_certificate->value);
	            return NGX_ERROR;
	        }

	        if (plcf->upstream.proxy_gmssl_enc_certificate->lengths
	            || plcf->upstream.proxy_gmssl_enc_key->lengths)
	        {
	            plcf->upstream.ssl_passwords =
	                  ngx_ssl_preserve_passwords(cf, plcf->upstream.ssl_passwords);
	            if (plcf->upstream.ssl_passwords == NULL) {
	            ...
	            if (ngx_gmssl_enc_certificate(cf, plcf->upstream.ssl,
	                                    &plcf->upstream.proxy_gmssl_enc_certificate->value,
	                                    &plcf->upstream.proxy_gmssl_enc_key->value,
	                                    plcf->upstream.ssl_passwords)
	                != NGX_OK)
	            {
	                return NGX_ERROR;
