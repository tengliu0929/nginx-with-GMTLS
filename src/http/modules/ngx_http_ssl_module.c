...
#endif
static char *
ngx_http_gmssl_enable( ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static ngx_conf_bitmask_t  ngx_http_ssl_protocols[] = {
    ...
    { ngx_string("GMSSL"),   NGX_SSL_GMSSL },
    { ngx_null_string, 0 }
};
...
static ngx_command_t  ngx_http_ssl_commands[] = {
	{ ngx_string("gmssl"),
	  ...
	  ngx_http_gmssl_enable,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  offsetof(ngx_http_ssl_srv_conf_t,gmssl_enable),
	  NULL },
    ...
    { ngx_string("ssl_certificate_key"),
	 ...
	 { ngx_string("ssl_sign_certificate"),
	   ...
	   ngx_conf_set_str_array_slot,
	   NGX_HTTP_SRV_CONF_OFFSET,
	   offsetof(ngx_http_ssl_srv_conf_t, gmssl_sign_certificates),
	   NULL },
	{ ngx_string("ssl_sign_certificate_key"),
	  NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE1,
	  ngx_conf_set_str_array_slot,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  offsetof(ngx_http_ssl_srv_conf_t, gmssl_sign_keys),
	  NULL },
	...
	{ ngx_string("ssl_enc_certificate"),
	  ...
	  ngx_conf_set_str_array_slot,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  offsetof(ngx_http_ssl_srv_conf_t, gmssl_enc_certificates ),
	  NULL },
	...
	{ ngx_string("ssl_enc_certificate_key"),
	  ...
	  ngx_conf_set_str_array_slot,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  offsetof(ngx_http_ssl_srv_conf_t, gmssl_enc_keys),
	  NULL },
...
ngx_http_ssl_create_srv_conf(ngx_conf_t *cf)
{
     ...
     */
	sscf->gmssl_enable = NGX_CONF_UNSET;
	...
	sscf->gmssl_enc_certificates = NGX_CONF_UNSET_PTR;
	sscf->gmssl_enc_keys = NGX_CONF_UNSET_PTR;
	sscf->gmssl_sign_certificates = NGX_CONF_UNSET_PTR;
	sscf->gmssl_sign_keys = NGX_CONF_UNSET_PTR;
...
ngx_http_ssl_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
	...
	//gmssl
	if(conf->gmssl_enable == NGX_CONF_UNSET) {
       if( prev->gmssl_enable == NGX_CONF_UNSET ){
	   	   conf->gmssl_enable = 0;
		} else {
          conf->gmssl_enable = prev->gmssl_enable;
    ...
    ngx_conf_merge_bitmask_value(conf->protocols, prev->protocols,
                         (NGX_CONF_BITMASK_SET
                          ...
                          |NGX_SSL_TLSv1_2|NGX_SSL_TLSv1_3 | NGX_SSL_GMSSL));
    ...
    ngx_conf_merge_ptr_value(conf->certificate_keys, prev->certificate_keys,
                         NULL);
    //gmssl
	ngx_conf_merge_ptr_value(conf->gmssl_sign_certificates, prev->gmssl_sign_certificates, NULL);
	ngx_conf_merge_ptr_value(conf->gmssl_sign_keys, prev->gmssl_sign_keys, NULL);

	ngx_conf_merge_ptr_value(conf->gmssl_enc_certificates, prev->gmssl_enc_certificates, NULL);
	ngx_conf_merge_ptr_value(conf->gmssl_enc_keys, prev->gmssl_enc_keys, NULL);
    ...
    // gmssl
    ngx_conf_merge_str_value(conf->client_gmssl_sign_certificate, prev->client_gmssl_sign_certificate, NULL);
	ngx_conf_merge_str_value(conf->client_gmssl_sign_keys, prev->client_gmssl_sign_keys, NULL);
	ngx_conf_merge_str_value(conf->client_gmssl_enc_certificate, prev->client_gmssl_enc_certificate, NULL);
	ngx_conf_merge_str_value(conf->client_gmssl_enc_keys, prev->client_gmssl_enc_keys,  NULL);
    ...
    if(!conf->gmssl_enable) {
       ...
       if(conf->gmssl_sign_certificates || conf->gmssl_enc_certificates )
       	{
       	   if(conf->gmssl_sign_keys == NULL 
		   	|| conf->gmssl_sign_keys->nelts < conf->gmssl_sign_certificates->nelts )
       	   	{
			   ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
							 "no \"gmssl_sign_certificate_key\" is defined "
							 "for certificate \"%V\"",
							 ((ngx_str_t *) conf->gmssl_sign_certificates->elts)
							 + conf->gmssl_sign_certificates->nelts - 1);

			   return NGX_CONF_ERROR;
       	   	}

		   if(conf->gmssl_enc_keys == NULL 
		   	 || conf->gmssl_enc_keys->nelts < conf->gmssl_enc_certificates->nelts )
		   	{
			  ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
										   "no \"gmssl_sign_certificate_key\" is defined "
										   "for certificate \"%V\"",
										   ((ngx_str_t *) conf->gmssl_enc_certificates->elts)
										   + conf->gmssl_enc_certificates->nelts - 1);
	...
	if(!conf->gmssl_enable) {
	  ...
	  if(ngx_gmssl_create(&conf->ssl, conf->protocols, conf) != NGX_OK ) {
           return NGX_CONF_ERROR;
	  }
	}
    ...
    if(!conf->gmssl_enable) {
       ...
       //sign
	   if( conf->gmssl_sign_certificate_values ) {
		 ...
		 ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
		               "variables in" 
		               "\"gmssl_sign_certificate\" and \"gmssl_sign_certificate_key\" "
		               "directives are not supported on this platform");
	     return NGX_CONF_ERROR;
#endif
	   	
	   } else if(conf->gmssl_sign_certificates) {

	     if(ngx_gmssl_sign_certificates(cf, &conf->ssl, conf->gmssl_sign_certificates,
		 							conf->gmssl_sign_keys,conf->passwords) 
		 	!= NGX_OK) {
		 	return NGX_CONF_ERROR;
		}
       ...
       // enc
	   if(conf->gmssl_enc_certificate_values ) {
		 ...
		 ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
		               "variables in" 
		               "\"gmssl_enc_certificate\" and \"gmssl_enc_certificate_key\" "
		               "directives are not supported on this platform");		 
         return NGX_CONF_ERROR;
#endif
	   } else if(conf->gmssl_enc_certificates ) {

	      if(ngx_gmssl_enc_certificates(cf,&conf->ssl, conf->gmssl_enc_certificates,
		  								conf->gmssl_enc_keys, conf->passwords) 
		  	!=NGX_OK) {
		  	return NGX_CONF_ERROR;
		 }
	   }
...
ngx_http_gmssl_enable( ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	//ngx_log_error(NGX_LOG_EMERG,cf->log,0,"sscf->gmssl_enable [%d]\n",sscf->gmssl_enable);
...
ngx_http_ssl_compile_certificates(ngx_conf_t *cf,
    ngx_http_ssl_srv_conf_t *conf)
{
    ngx_str_t                         *cert = NULL, *key = NULL;
	ngx_str_t                         *gm_sign_cert = NULL, *gm_sign_key = NULL;
	ngx_str_t                         *gm_enc_cert = NULL,  *gm_enc_key = NULL;
    ngx_uint_t                         i, nelts;
   ...
   if(!conf->gmssl_enable)
      ...
      if(conf->gmssl_sign_certificates == NULL )
	  	return NGX_OK;

	   gm_sign_cert = conf->gmssl_sign_certificates->elts;
	   gm_sign_key = conf->gmssl_sign_keys->elts;
	   gm_enc_cert = conf->gmssl_enc_certificates->elts;
	   gm_enc_key = conf->gmssl_enc_keys->elts;
	   nelts = conf->gmssl_sign_certificates->nelts;

	   for( i = 0; i < nelts; i++ )
	   	{
	   	  if( ngx_http_script_variables_count(&gm_sign_cert[i]))
		  	{
		  	  goto found;
			  
		  	}
		  if( ngx_http_script_variables_count(&gm_sign_key[i]))
		  	{
		  	  goto found;
		  	}

		  if( ngx_http_script_variables_count(&gm_enc_cert[i]) )
		  	{
		  	  goto found;
		  	}

		  if( ngx_http_script_variables_count(&gm_enc_key[i]) )
		  	{
		  	  goto found;
		  	}
	   	}
...
found:
	...
	{
	   conf->gmssl_sign_certificate_values = ngx_array_create(cf->pool, nelts,
	   															sizeof(ngx_http_complex_value_t));
	   if( conf->gmssl_sign_certificate_values == NULL)
	   	return NGX_ERROR;

	   conf->gmssl_sign_key_values = ngx_array_create(cf->pool, nelts,
	   															sizeof(ngx_http_complex_value_t));
	   if(conf->gmssl_sign_key_values ==  NULL)
	   	return NGX_ERROR;

	   conf->gmssl_enc_certificate_values = ngx_array_create(cf->pool, nelts,
	   															 sizeof(ngx_http_complex_value_t));
	   if(conf->gmssl_enc_certificate_values == NULL)
	   	return NGX_ERROR;

	  conf->gmssl_enc_key_values = ngx_array_create(cf->pool, nelts,
	  															sizeof(ngx_http_complex_value_t));
	   if(conf->gmssl_enc_key_values == NULL )
	   	return NGX_ERROR;

	  for( i = 0; i < nelts; i++ )
	  {
	    // sign 
	    cv = ngx_array_push(conf->gmssl_sign_certificate_values);
		if(cv == NULL){
		  return NGX_ERROR;
		}
		...
		cv = ngx_array_push(conf->gmssl_sign_key_values);
		if(cv == NULL) {
		  return NGX_ERROR;
		}
		...
		//enc
        cv = ngx_array_push(conf->gmssl_enc_certificate_values);
		if( cv == NULL ) {
         return NGX_ERROR;
		}
        ...
        cv = ngx_array_push(conf->gmssl_enc_key_values);
		if( cv == NULL ){
         return NGX_ERROR;
		}
...
ngx_http_ssl_init(ngx_conf_t *cf)
{
    ...
    for (p = 0; p < cmcf->ports->nelts; p++) {
        ...
        for (a = 0; a < port[p].addrs.nelts; a++) {
             ...
             */
            if(!sscf->gmssl_enable)
            {
                if (!sscf->reject_handshake) {
                ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                              "no \"ssl_certificate\" is defined for "
                              "the \"listen ... %s\" directive in %s:%ui reject_handshake",
                              name, cscf->file_name, cscf->line);
                return NGX_ERROR;
            }
	            ...
	            for (s = 0; s < addr[a].servers.nelts; s++) {
	                ...
	                ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
	                              "no \"ssl_certificate\" is defined for "
	                              "the \"listen ... %s\" directive in %s:%ui no gmssl_enable",
	                              name, cscf->file_name, cscf->line);
	                return NGX_ERROR;
	            }
        	}
