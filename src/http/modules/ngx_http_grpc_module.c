...
static ngx_command_t  ngx_http_grpc_commands[] = {
    ...
    { ngx_string("grpc_ssl_certificate_key"),
      ...
      NULL },
    // gmssl
    { ngx_string("grpc_gmssl_sign_certificate"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_grpc_loc_conf_t, upstream.proxy_gmssl_sign_certificate),
      NULL },

    { ngx_string("grpc_gmssl_sign_key"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_grpc_loc_conf_t, upstream.proxy_gmssl_sign_key),
      NULL },
	    { ngx_string("grpc_gmssl_enc_certificate"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_grpc_loc_conf_t, upstream.proxy_gmssl_enc_certificate),
      NULL },

    { ngx_string("grpc_gmssl_enc_key"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_set_complex_value_zero_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_grpc_loc_conf_t, upstream.proxy_gmssl_enc_key),
      NULL },
...
ngx_http_grpc_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
	...
	//gmssl
	ngx_conf_merge_value(conf->gmssl_enable, prev->gmssl_enable, NULL);
	ngx_conf_merge_ptr_value(conf->upstream.proxy_gmssl_sign_certificate, prev->upstream.proxy_gmssl_sign_certificate, NULL);
	ngx_conf_merge_ptr_value(conf->upstream.proxy_gmssl_sign_key, prev->upstream.proxy_gmssl_sign_key, NULL);
...
ngx_http_grpc_set_ssl(ngx_conf_t *cf, ngx_http_grpc_loc_conf_t *glcf)
{
    ...
    if(!glcf->gmssl_enable) {
       ...
       if(ngx_gmssl_create(glcf->upstream.ssl, glcf->ssl_protocols, NULL) != NGX_OK)
	   {
	     return NGX_ERROR;
	   }
	}
	...
	// sign
	   if (glcf->upstream.proxy_gmssl_sign_certificate
	        && glcf->upstream.proxy_gmssl_sign_certificate->value.len)
	    {
	        if (glcf->upstream.proxy_gmssl_sign_key == NULL) {
	            ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
	                          "no \"grpc_ssl_certificate_key\" is defined "
	                          "for certificate \"%V\"",
	                          &glcf->upstream.proxy_gmssl_sign_certificate->value);
	            return NGX_ERROR;
	        }

	        if (glcf->upstream.proxy_gmssl_sign_certificate->lengths
	            || glcf->upstream.proxy_gmssl_sign_key->lengths)
	        {
	            glcf->upstream.ssl_passwords =
	                  ngx_ssl_preserve_passwords(cf, glcf->upstream.ssl_passwords);
	            if (glcf->upstream.ssl_passwords == NULL) {
	            ...
	            if (ngx_gmssl_sign_certificate(cf, glcf->upstream.ssl,
	                                    &glcf->upstream.proxy_gmssl_sign_certificate->value,
	                                    &glcf->upstream.proxy_gmssl_sign_key->value,
	                                    glcf->upstream.ssl_passwords)
	                != NGX_OK)
	            {
	                return NGX_ERROR;
	...
	//enc
    	    if (glcf->upstream.proxy_gmssl_enc_certificate
	        && glcf->upstream.proxy_gmssl_enc_certificate->value.len)
	    {
	        if (glcf->upstream.proxy_gmssl_enc_key == NULL) {
	            ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
	                          "no \"grpc_ssl_certificate_key\" is defined "
	                          "for certificate \"%V\"",
	                          &glcf->upstream.proxy_gmssl_enc_certificate->value);
	            return NGX_ERROR;
	        }

	        if (glcf->upstream.proxy_gmssl_enc_certificate->lengths
	            || glcf->upstream.proxy_gmssl_enc_key->lengths)
	        {
	            glcf->upstream.ssl_passwords =
	                  ngx_ssl_preserve_passwords(cf, glcf->upstream.ssl_passwords);
	            if (glcf->upstream.ssl_passwords == NULL) {
	            ...
	            if (ngx_gmssl_enc_certificate(cf, glcf->upstream.ssl,
	                                    &glcf->upstream.proxy_gmssl_enc_certificate->value,
	                                    &glcf->upstream.proxy_gmssl_enc_key->value,
	                                    glcf->upstream.ssl_passwords)
	                != NGX_OK)
	            {
	                return NGX_ERROR;
