...
typedef struct {
    ...
    //gmssl
    ngx_flag_t                       proxy_gmssl_enable;
	ngx_stream_complex_value_t       *proxy_sign_certificate;
	ngx_stream_complex_value_t       *proxy_sign_key;
	ngx_stream_complex_value_t       *proxy_enc_certificate;
	ngx_stream_complex_value_t       *proxy_enc_key;
...
static ngx_command_t  ngx_stream_proxy_commands[] = {
    ...
    { ngx_string("proxy_ssl"),
      ...
      NULL },
     //gmssl
	{ ngx_string("proxy_gmssl_enable"),
	  NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_FLAG,
	  ngx_conf_set_flag_slot,
	  NGX_STREAM_SRV_CONF_OFFSET,
	  offsetof(ngx_stream_proxy_srv_conf_t, proxy_gmssl_enable),
	  NULL },
    ...
    { ngx_string("proxy_sign_certificate"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_stream_set_complex_value_zero_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_proxy_srv_conf_t, proxy_sign_certificate),
      NULL },

    { ngx_string("proxy_sign_key"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_stream_set_complex_value_zero_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_proxy_srv_conf_t, proxy_sign_key),
      NULL },
     
    { ngx_string("proxy_enc_certificate"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_stream_set_complex_value_zero_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_proxy_srv_conf_t, proxy_enc_certificate),
      NULL },

    { ngx_string("proxy_enc_key"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_stream_set_complex_value_zero_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_proxy_srv_conf_t, proxy_enc_key),
      NULL },
...
ngx_stream_proxy_ssl_init_connection(ngx_stream_session_t *s)
{
	...
	if(!pscf->proxy_gmssl_enable) {
	            ...
	            || pscf->ssl_certificate_key->lengths))
	    {
		 ...
		 if ( pscf->proxy_sign_certificate
	        && pscf->proxy_sign_certificate->value.len
	        && (pscf->proxy_sign_certificate->lengths
	            || pscf->proxy_sign_key->lengths) && 
	            pscf->proxy_enc_certificate
	        && pscf->proxy_enc_certificate->value.len
	        && (pscf->proxy_enc_certificate->lengths
	            || pscf->proxy_enc_key->lengths) )
	    {
	        if (ngx_stream_proxy_ssl_certificate(s) != NGX_OK) {
	            ngx_stream_proxy_finalize(s, NGX_STREAM_INTERNAL_SERVER_ERROR);
	            return;
...
ngx_stream_proxy_ssl_certificate(ngx_stream_session_t *s)
{
	...
	if(!pscf->proxy_gmssl_enable) {
      ...
      // sign
      	    if (ngx_stream_complex_value(s, pscf->proxy_sign_certificate, &cert)
	        != NGX_OK)
	    {
	        return NGX_ERROR;
	    }
	    ...
	    if (ngx_stream_complex_value(s, pscf->proxy_sign_key, &key)
	        != NGX_OK)
	    {
	        return NGX_ERROR;
	    }
	  ...
	  // enc
       	    if (ngx_stream_complex_value(s, pscf->proxy_enc_certificate, &cert)
	        != NGX_OK)
	    {
	        return NGX_ERROR;
	    }
...
ngx_stream_proxy_create_srv_conf(ngx_conf_t *cf)
{
	...
	//gmssl
	conf->proxy_gmssl_enable = NGX_CONF_UNSET;
	conf->proxy_sign_certificate = NGX_CONF_UNSET_PTR;
	conf->proxy_sign_key = NGX_CONF_UNSET_PTR;
	conf->proxy_enc_certificate = NGX_CONF_UNSET_PTR;
	conf->proxy_enc_key = NGX_CONF_UNSET_PTR;
#endif
...
ngx_stream_proxy_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
	...
	//gmssl
	ngx_conf_merge_value(conf->proxy_gmssl_enable, prev->proxy_gmssl_enable, 0);
	...
	//gmssl
	ngx_conf_merge_ptr_value(conf->proxy_sign_certificate, conf->proxy_sign_certificate, NULL);
	ngx_conf_merge_ptr_value(conf->proxy_sign_key, conf->proxy_sign_key, NULL);
	ngx_conf_merge_ptr_value(conf->proxy_enc_certificate, conf->proxy_enc_certificate, NULL);
	ngx_conf_merge_ptr_value(conf->proxy_enc_key, conf->proxy_enc_key, NULL);
...
ngx_stream_proxy_set_ssl(ngx_conf_t *cf, ngx_stream_proxy_srv_conf_t *pscf)
{
	...
	if(!pscf->proxy_gmssl_enable) {
	    ...
	    if(ngx_gmssl_create(pscf->ssl, pscf->ssl_protocols, NULL) != NGX_OK) {
			return NGX_ERROR;
		}
	}
	...
	if(!pscf->proxy_gmssl_enable) {
	    ...
	    if (pscf->proxy_sign_certificate
	        && pscf->proxy_sign_certificate->value.len)
	    {
	        if (pscf->proxy_sign_key == NULL) {
	            ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
	                          "no \"proxy_sign_certificate\" is defined "
	                          "for certificate \"%V\"",
	                          &pscf->proxy_sign_key->value);
	            return NGX_ERROR;
	        }

	        if (pscf->proxy_sign_certificate->lengths
	            || pscf->proxy_sign_key->lengths)
	        {
	            pscf->ssl_passwords =
	                           ngx_ssl_preserve_passwords(cf, pscf->ssl_passwords);
	            ...
	            if (ngx_gmssl_sign_certificate(cf, pscf->ssl,
	                                    &pscf->proxy_sign_certificate->value,
	                                    &pscf->proxy_sign_key->value,
	                                    pscf->ssl_passwords)
	                != NGX_OK)
	            {
		...
		// enc
		if (pscf->proxy_enc_certificate
	        && pscf->proxy_enc_certificate->value.len)
	    {
	        if (pscf->proxy_enc_key == NULL) {
	            ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
	                          "no \"proxy_enc_key\" is defined "
	                          "for certificate \"%V\"",
	                          &pscf->proxy_enc_certificate->value);
	            return NGX_ERROR;
	        }

	        if (pscf->proxy_enc_certificate->lengths
	            || pscf->proxy_enc_key->lengths)
	        {
	            pscf->ssl_passwords =
	                           ngx_ssl_preserve_passwords(cf, pscf->ssl_passwords);
	            ...
	            if (ngx_gmssl_enc_certificate(cf, pscf->ssl,
	                                    &pscf->proxy_enc_certificate->value,
	                                    &pscf->proxy_enc_key->value,
	                                    pscf->ssl_passwords)
	                != NGX_OK)
	            {
	                return NGX_ERROR;
