...
static int ngx_stream_gmssl_certificate(ngx_ssl_conn_t *ssl_conn, void *arg);
...
static ngx_command_t  ngx_stream_ssl_commands[] = {
    ...
    { ngx_string("ssl_certificate_key"),
      ...
      NULL },
     // gmssl
    { ngx_string("gmssl_enable"),
      NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
      ngx_conf_set_flag_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_conf_t, gmssl_enable),
     NULL },

	{ ngx_string("gmssl_sign_certificate"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_array_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_conf_t, gmssl_sign_certificates),
      NULL },
    { ngx_string("gmssl_sign_key"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_array_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_conf_t, gmssl_sign_keys),
      NULL },
    { ngx_string("gmssl_enc_certificate"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_array_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_conf_t, gmssl_enc_certificates),
      NULL },
    { ngx_string("gmssl_enc_key"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_array_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_conf_t, gmssl_enc_keys),
      NULL },
...
static int
ngx_stream_gmssl_certificate(ngx_ssl_conn_t *ssl_conn, void *arg)
{
    ngx_str_t                    cert, key;
    ngx_uint_t                   i, nelts;
    ngx_connection_t            *c;
    ...
    sslcf = arg;
    //sign
    nelts = sslcf->gmssl_sign_certificate_values->nelts;
    certs = sslcf->gmssl_sign_certificate_values->elts;
    keys = sslcf->gmssl_sign_key_values->elts;
    ...
    for (i = 0; i < nelts; i++) {
        ...
        if (ngx_gmssl_sign_connection_certificate(c, c->pool, &cert, &key,
                                           sslcf->passwords)
            != NGX_OK)
        {
            return 0;
   ...
   //enc
    ...
    nelts = sslcf->gmssl_enc_certificate_values->nelts;
    certs = sslcf->gmssl_enc_certificate_values->elts;
    keys = sslcf->gmssl_enc_key_values->elts;
    ...
    for (i = 0; i < nelts; i++) {
        ...
        if (ngx_gmssl_enc_connection_certificate(c, c->pool, &cert, &key,
                                           sslcf->passwords)
            != NGX_OK)
        {
            return 0;
...
ngx_stream_ssl_create_conf(ngx_conf_t *cf)
{
	...
	//gmssl
	scf->gmssl_enable = NGX_CONF_UNSET;
	scf->gmssl_sign_certificates = NGX_CONF_UNSET_PTR;
	scf->gmssl_sign_keys = NGX_CONF_UNSET_PTR;
    scf->gmssl_enc_certificates = NGX_CONF_UNSET_PTR;
	scf->gmssl_enc_keys = NGX_CONF_UNSET_PTR;
    return scf;
}
...
ngx_stream_ssl_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_ptr_value(conf->certificate_keys, prev->certificate_keys,
                         NULL);
	//gmssl
	ngx_conf_merge_value(conf->gmssl_enable, prev->gmssl_enable, 0);
	...
	if(!conf->gmssl_enable) {
     ...
     if (conf->gmssl_sign_certificates == NULL) {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                      "no \"gmssl_sign_certificates\" is defined for "
                      "the \"listen ... ssl\" directive in %s:%ui",
                      conf->file, conf->line);
        return NGX_CONF_ERROR;
    }
    ...
    if (conf->gmssl_sign_keys == NULL) {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                      "no \"gmssl_sign_keys\" is defined for "
                      "the \"listen ... ssl\" directive in %s:%ui",
                      conf->file, conf->line);
        return NGX_CONF_ERROR;
    }

    if (conf->gmssl_sign_keys->nelts < conf->gmssl_sign_certificates->nelts) {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                      "no \"gmssl_sign_keys\" is defined "
                      "for certificate \"%V\" and "
                      "the \"listen ... ssl\" directive in %s:%ui",
    ...
    //enc
   if (conf->gmssl_enc_certificates == NULL) {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                      "no \"gmssl_enc_certificates\" is defined for "
                      "the \"listen ... ssl\" directive in %s:%ui",
                      conf->file, conf->line);
        return NGX_CONF_ERROR;
    }
    ...
    if (conf->gmssl_enc_keys == NULL) {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                      "no \"gmssl_enc_keys\" is defined for "
                      "the \"listen ... ssl\" directive in %s:%ui",
                      conf->file, conf->line);
        return NGX_CONF_ERROR;
    }

    if (conf->gmssl_enc_keys->nelts < conf->gmssl_enc_certificates->nelts) {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                      "no \"gmssl_enc_keys\" is defined "
                      "for certificate \"%V\" and "
                      "the \"listen ... ssl\" directive in %s:%ui",
    ...
    if (ngx_gmssl_create(&conf->ssl, conf->protocols, NULL) != NGX_OK) {
        return NGX_CONF_ERROR;
    }
	...
	if(!conf->gmssl_enable) {
	    if (conf->certificate_values) {
	            ...
	            != NGX_OK)
	        {
      	    ...
      	    if (conf->gmssl_sign_key_values != NULL && conf->gmssl_enc_key_values != NULL ) {

#ifdef SSL_R_CERT_CB_ERROR

	        /* install callback to lookup certificates */

	        SSL_CTX_set_cert_cb(conf->ssl.ctx, ngx_stream_gmssl_certificate, conf);
    ...
    ngx_stream_ssl_conf_t *conf)
{
    ngx_str_t                           *cert, *key;
	//gmssl
	//ngx_str_t                           *sign_cert, *sign_key;
	//ngx_str_t                           *enc_cert, *enc_key;
    ngx_uint_t                           i, nelts;
    ...
    if(!conf->gmssl_enable) {
	    ...
	    for (i = 0; i < nelts; i++) {
	   	...
	   	cert = conf->gmssl_sign_certificates->elts;
	    key = conf->gmssl_sign_keys->elts;
	    ...
	    cert = conf->gmssl_enc_certificates->elts;
	    key = conf->gmssl_enc_keys->elts;
	    nelts = conf->gmssl_enc_certificates->nelts;
